...
Motor Slope Controll
 1) 0 → 255 (점차 증가)
 2) 255 → 0.8s (변경될 수 있음)
 3) 255 → 150 (점차 감소)
 4) 150 → 0.2s (변경될 수 있음)
 5) 150 → 0 (점차 감소)
 * 어느 단계든 limit sw 눌리면 정지

- 0114일자 수정
1. limit sw 핀 변경 기존 17 under → 8, 18 top → 11
2. pwm, dir 핀 바뀜 배선 잘못함 원래는 DIR이 10, PWM이 9가 맞음 그러니 배선 보고 연결잘 되면 기존 pin set으로 교체
3. 내려갈 때는 슬로프해서 마지막에 느려져서 눌리는데 올라올 때는 슬로프 하기는 하는데 느려진 상태로 눌려지지 않음 올라올 때 풀파워 유지 시간 변경 필요
   단, 마네킹을 올리지 않았을 때임
4. mnq_boot_home_once() 함수 mnq 바스켓이 올라와 있지 않을 때 어디에 있을지 모르니 그냥 pwm 150 고정이 맞는 것 같음 수정 필요
...
// ACT
#define MNQ_DIR         9  // DIR 기존 10
#define MNQ_PWM         10   // PWM 기존 9
#define MNQ_HIGH        1
#define MNQ_LOW         0

// limit sw (normal state = HIGH, press state = LOW)
#define LIMIT_SW_UNDER  8  // under → 내려갈 때 눌리면 정지
#define LIMIT_SW_TOP    11  // top   → 올라갈 때 눌리면 정지
...
// 풀파워 유지 시간 (1000ms = 1s)
#define FULL_POWER_MS_DOWN  1000    // 내려가기 까지 약 1.2초 소요
#define FULL_POWER_MS_UP    1000    // 올라가기 까지 약 1.7초 소요
    ...
    while (true) {
        ...
        if (mnq_interrupt_flag){
            ...
            if (Readsignal_detect_1()){
                ...
                gpio_put(LED, 0);
                hits_clear();
                acting_mnq(2);      // test 용 즉시 넘어가기로 하였는데 추후 수정 필요
            }
...
static void gpio_setup(void) {
    ...
    gpio_pull_down(LIMIT_SW_UNDER);

    gpio_init(LIMIT_SW_TOP);
    gpio_set_dir(LIMIT_SW_TOP, GPIO_IN);
    gpio_pull_down(LIMIT_SW_TOP);
...
static void mnq_boot_home_once(void)
{
    ...
    */
    ...
    // tow_sw가 눌려 있음 = MNQ는 올라와 있으니 넘어가기 눌려져 있지 않으면 올라오게 하기
    if (gpio_get(LIMIT_SW_TOP) == 0) {
        pwm_set_gpio_level(MNQ_PWM, 0);
        return;
    } 
