...
/*
P05 / P05L motor_slope_control ver

sw 상태
올라갈 때 아래(하) sw가 눌림 (mnq 올라온 상태)
내려갈 때 위(상)) sw가 눌림 (mnq 내려간 상태)
...
// ACT
#define MNQ_DIR         8   // DIR
#define MNQ_PWM         9   // PWM
#define MNQ_HIGH        1
#define MNQ_LOW         0

// limit sw (normal state = HIGH, press state = LOW)
#define LIMIT_SW_UNDER  17  // under → 올라갈 때 눌리면 정지
#define LIMIT_SW_TOP    18  // top   → 내려갈 때 눌리면 정지
...
static void acting_mnq(int mnq_move)
{
    // mnq_move: 2=headshot 즉시 down, body_shot_count 2회 이상이면 down, 0=무시
    bool start = false;
    if (mnq_move == 2) {
        start = true;
    }
    ...
    if (start) {
        ...
        // DOWN 시퀀스
        // =========================
        gpio_put(MNQ_DIR, MNQ_HIGH);           // HIGH=down
        pwm_set_gpio_level(MNQ_PWM, 0);
        ...
        // 3) 리밋(또는 타임아웃) -> 150->0 브레이크 (1ms에 3씩)
        pwm = LOW_PWM;
        while (pwm > 0) {
            if (gpio_get(LIMIT_SW_TOP) == 0) break;
            pwm -= PWM_BRAKE_STEP;
        ...
        // UP 시퀀스
        // =========================
        gpio_put(MNQ_DIR, MNQ_LOW);            // LOW=up
        pwm_set_gpio_level(MNQ_PWM, 0);
        pwm = 0;
...
static void mnq_boot_home_once(void)
{
    ...
    */
    ...
    uint32_t t0 = now_ms();
    while ((now_ms() - t0) < FULL_POWER_MS_UP) {
        if (gpio_get(LIMIT_SW_UNDERP) == 0) break;
        busy_wait_ms(1);
    }
