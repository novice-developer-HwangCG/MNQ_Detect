...
volatile uint8_t default_cri_set = 1;
volatile uint8_t cur_cri_set = 1;
volatile uint8_t default_non_cri_set = 1;
volatile uint8_t cur_non_cri_set = 1;
volatile uint8_t hit_count = 0;
        ...
        */
        if (Receive_Done > 0){
            if (Receive_Data[0] == 0){
                ...
                if (Receive_Data[1] == 0){
                    if ((hit_channel == 1 && first_channel_ban) ||
                        (hit_channel == 2 && second_channel_ban) ||
                        (hit_channel == 3 && third_channel_ban)) {
                        hit_channel = 0;
                    }
                    //자동
                    if (hit_channel != 0 && act == 0 && !up_wait) {

                        acting_mnq_motor(1);   // DOWN
                        up_wait = true;        // 나중에 자동 UP 예약
                    }
                }
                else {
                    up_wait = false;
        ...
        // if (gpio_get(LIMIT_TOP) == 1) {
        //     if (gpio_get_out_level(MNQ_DIR) == DIR_UP) {
        //         pwm_set_gpio_level(MNQ_PWM, 255);
        //     }
        // }
        // else if (gpio_get(LIMIT_UNDER) == 1) {
        //     if (gpio_get_out_level(MNQ_DIR) == DIR_DOWN) {
        //         pwm_set_gpio_level(MNQ_PWM, 255);
        //     }
        // }
...
void Tx_uart_SBC(int hit_channel)
{
    sprintf(buffer, "%d\n", hit_channel);
...
// 모터가 움직일 시간을 주기 위한 별도 타이머 같은데 함수 의미 파악해서 사용할 것
void start_motor_timer()
{
    motor_active = true;
