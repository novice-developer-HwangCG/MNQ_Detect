...
#define HIT_BLANK_MS 50

static uint32_t last_hit_ms_ch1 = 0;
static uint32_t last_hit_ms_ch2 = 0;
static uint32_t last_hit_ms_ch3 = 0;

// legacy 변수 - 현재 로직에서는 미사용
...
static void gpio_irq_callback(uint gpio, uint32_t events) {
    ...
    if (gpio == DETECT_1 && gpio_get(DETECT_1)) {
        if ((now - last_hit_ms_ch1) < HIT_BLANK_MS) return;
        last_hit_ms_ch1 = now;
        ...
        if ((now - last_hit_ms_ch2) < HIT_BLANK_MS) return;
        last_hit_ms_ch2 = now;
        ...
        if ((now - last_hit_ms_ch3) < HIT_BLANK_MS) return;
        last_hit_ms_ch3 = now;
...
void uart_timeout(){
    if (absolute_time_diff_us(last_uart_time, get_absolute_time()) >= check_uart_time_out) {
        ...
        Receive_Done = 0;                // 추가
        Receive_Data_Count = 0;          // 추가
        memset(Receive_Data, 0, sizeof(Receive_Data));   // 추가
